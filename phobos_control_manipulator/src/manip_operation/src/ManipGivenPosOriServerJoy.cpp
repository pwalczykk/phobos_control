...
void ManipGivenPosOriServerJoy::ProcessInput(std::vector<float>* axes, std::vector<int>* buttons){
	...
	if(!rollgrip_mode){
		...
		this->x_given += (*axes)[4] * joystick_sensitivity;
		this->y_given += (*axes)[3] * joystick_sensitivity;
		this->z_given += (*axes)[1] * joystick_sensitivity;
		this->angle_given += (*axes)[0] * joystick_sensitivity;
	}
	
	else{
		
		this->roll_given += (*axes)[3] * joystick_sensitivity;
